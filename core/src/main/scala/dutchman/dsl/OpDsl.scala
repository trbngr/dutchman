...
trait OpDsl {
  ...
  case class Index(index: Idx, `type`: Type, document: ElasticDocument, version: Option[Int]) extends ElasticOp[IndexResponse] with BulkActionable
  ...
  case class Update(index: Idx, `type`: Type, document: ElasticDocument) extends ElasticOp[UpdateResponse] with BulkActionable
  case class UpdateResponse()

  case class MultiGet[Json](ids: Seq[(Idx, Option[Type], Option[Id])]) extends ElasticOp[MultiGetResponse[Json]]
  case class MultiGetResponse[Json]()

  case class Bulk(actions: Seq[(BulkAction, BulkActionable)]) extends ElasticOp[Seq[BulkResponse]]
  case class BulkResponse(action: BulkAction, status: Int, response: Response)

  case class DeleteIndex(index: Idx) extends ElasticOp[DeleteIndexResponse]
  case class DeleteIndexResponse(acknowledged: Boolean)

  case class Refresh(indices: Seq[Idx]) extends ElasticOp[RefreshResponse]
